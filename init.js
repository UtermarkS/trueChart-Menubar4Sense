...
define(['qlik', 'qvangular', 'jquery', 'ng!$timeout', './translations/' + lang + '/translations', './lib/hico/services/qlik-service'], function (qlik, qvangular, $, $timeout, translation, QlikService) {
  ...
  */
   ...
   */
		...
		function initListObjects(layout) {
			var i,
			    j,
			    listItem,
			    selectItem,
		...
		function updateListObjects(layout) {
			destroyListObjects().then(function () {
				initListObjects(layout);
			});
		}
   ...
   */
		...
		function destroyListObjects() {
			var promises = [];

			removeListener(_eventListener);
			_eventListener = [];

			$scope.listItemsDub && $scope.listItemsDub.forEach(function (listItem) {
  ...
  */
	function paint($element, layout) {
		...
		if (($scope.listItemsDub.length === 0 || $scope.wasEditMode) && !$scope.inEditMode()) {
			...
			if (setDimensionForListItems(layout, $scope.listItemsDub)) {

				layout = setShowPropertiesToLayout(layout, $scope.listItemsDub);

				$scope.updateListObjects(layout);
	...
	//returns a boolean if something changed
	function setDimensionForListItems(layout, listItemsDub) {
		var somethingChanged = false;
		layout.listItems.forEach(function (listItem) {
			if (listItem.type === "Single Select" || listItem.type === "Sense Select") {
				...
				!somethingChanged && listItemsDub.some(function (item) {
					if (item.props.dimTitle === listItem.props.dimTitle) {
						return somethingChanged = item.props.dim !== listItem.props.dim;
					}
				});
			}

			(listItem.selectItems || []).forEach(function (selectItem) {
				...
				!somethingChanged && listItemsDub.some(function (item) {
					if (item.props.dimTitle === selectItem.props.dimTitle) {
						return somethingChanged = item.props.dim !== selectItem.props.dim;
					}
				});
			});
		});
	...
	function setShowPropertiesToLayout(layout, listItems) {
